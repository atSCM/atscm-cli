...
class InitCommand extends _Command2.default {
  ...
  constructor(name, description) {
    super(name, description, {
      options: {
        yes: _Options2.default.yes,
        force: _Options2.default.force,
        beta: _Options2.default.beta,
        link: _Options2.default.link
      }
    });
  }
    ...
    return new Promise((resolve, reject) => {
      (0, _fs.readdir)(path, (err, files) => {
        if (err) {
          ...
          const message = `${_Logger2.default.format.path(path)} is not empty`;

          if (overwrite) {
            _Logger2.default.warn(message);
            _Logger2.default.warn(_Logger2.default.colors.yellow('Using --force, continue...'));
            resolve(path);
          } else {
  ...
  runNpm(args, options = {}) {
    return new Promise((resolve, reject) => {
      (0, _which2.default)('npm', (err, npm) => {
        ...
        const child = (0, _child_process.spawn)(npm, args, Object.assign({}, options, {/* stdio: 'inherit' */})).on('error', npmErr => reject(npmErr)).on('close', code => {
          if (code > 0) {
        ...
        _Logger2.default.pipeLastLine(child.stderr);
  ...
  async installLocal(path, { beta: useBetaRelease = false, link = false } = {}) {
    ...
    if (useBetaRelease) {
      _Logger2.default.debug(_Logger2.default.colors.gray('Using beta release'));
    }

    await this.install(path, useBetaRelease ? 'atscm@beta' : 'atscm');

    if (link) {
      _Logger2.default.info('Linking atscm...');
  ...
  checkCliVersion(env) {
    ...
    if (!(0, _semver.satisfies)(_package2.default.version.split('-beta')[0], required)) {
