 ...
 */
class DocsCommand extends _Command.default {
  /**
  ...
  constructor(name, description) {
    super(name, description, {
      options: {
        cli: _Options.default.cli,
        browser: _Options.default.browser,
        remote: _Options.default.remote
      },
      maxArguments: 0
    });
  }
  ...
  remoteDocsUrl(cli) {
    ...
    return (0, _url.resolve)(this.constructor.RemoteDocsBase, path);
  }
  /**
  ...
  run(cli) {
    return (!cli.options.cli && !cli.environment ? cli.getEnvironment() : Promise.resolve()).then(() => this.addressToOpen(cli)).then(({
      address,
      isPath
    }) => {
      _Logger.default.debug('Opening', isPath ? _Logger.default.format.path(address) : address);

      (0, _open.default)(address, cli.options.browser ? {
        app: cli.options.browser
      } : undefined);
    });
  }
  /**
