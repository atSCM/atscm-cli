...
class InitCommand extends _Command.default {
  /**
  ...
  constructor(name, description) {
    super(name, description, {
      options: {
        yes: _Options.default.yes,
        force: _Options.default.force,
        beta: _Options.default.beta,
        link: _Options.default.link
      }
    });
  }
  /**
  ...
  checkDirectory(path, overwrite = false) {
    return new Promise((resolve, reject) => {
      (0, _fs.readdir)(path, (err, files) => {
        if (err) {
          if (err.code === 'ENOENT') {
            reject(new Error(`${_Logger.default.format.path(path)} does not exist`));
          } else if (err.code === 'ENOTDIR') {
            reject(new Error(`${_Logger.default.format.path(path)} is not a directory`));
          } else {
            reject(err);
          }
        } else if (files.filter(f => !IgnoredFiles.includes(f.toLowerCase())).length > 0) {
          const message = `${_Logger.default.format.path(path)} is not empty`;

          if (overwrite) {
            _Logger.default.warn(message);

            _Logger.default.warn(_Logger.default.colors.yellow('Using --force, continue...'));
  ...
  runNpm(args, options = {}) {
    return new Promise((resolve, reject) => {
      (0, _which.default)('npm', (err, npm) => {
        if (err) {
          return reject(err);
        }

        const child = (0, _child_process.spawn)(npm, args, Object.assign({}, options, {
          /* stdio: 'inherit' */
        })).on('error', npmErr => reject(npmErr)).on('close', code => {
          if (code > 0) {
        ...
        _Logger.default.pipeLastLine(child.stderr);

        _Logger.default.pipeLastLine(child.stdout);
  ...
  install(path, packages) {
    return this.runNpm(['install', '--save-dev'].concat(packages), {
      cwd: path
    });
  }
  /**
  ...
  async installLocal(path, {
    beta: useBetaRelease = false,
    link = false
  } = {}) {
    _Logger.default.info('Installing latest version of atscm...');

    if (useBetaRelease) {
      _Logger.default.debug(_Logger.default.colors.gray('Using beta release'));
    }

    await this.install(path, useBetaRelease ? 'atscm@beta' : 'atscm');

    if (link) {
      _Logger.default.info('Linking atscm...');

      await this.runNpm(['link', 'atscm'], {
        cwd: path
      });
    }
  }
  ...
  checkCliVersion(env) {
    _Logger.default.debug('Checking atscm-cli version...');

    const required = env.modulePackage.engines['atscm-cli'];

    if (!(0, _semver.satisfies)(_package.default.version.split('-beta')[0], required)) {
      _Logger.default.info('Your version of atscm-cli is not compatible with the latest version atscm.');

      _Logger.default.info('Please run', _Logger.default.format.command('npm install -g atscm-cli'), 'to update.');
  ...
  getOptions(modulePath, {
    ...
    const options = defaultExport(require((0, _path.join)(modulePath, '../init/options')));

    if (useDefaults) {
      return this.getDefaultOptions(options);
    }

    _Logger.default.info('Answer these questions to create a new project:');
  ...
  writeFiles(modulePath, options) {
    // eslint-disable-next-line global-require
    return defaultExport(require((0, _path.join)(modulePath, '../init/init')))(options);
  }
  /**
  ...
  installDependencies(path, deps) {
    _Logger.default.info('Installing dependencies...');
  ...
  run(cli) {
    return cli.getEnvironment(false).then(env => this.checkDirectory(env.cwd, cli.options.force)).then(() => this.createEmptyPackage(cli.environment.cwd)).then(() => this.installLocal(cli.environment.cwd, cli.options)).then(() => cli.getEnvironment(false)).then(env => this.checkCliVersion(env)).then(env => process.chdir(env.cwd)).then(() => this.getOptions(cli.environment.modulePath, {
      useDefaults: cli.options.yes
    })).then(options => this.writeFiles(cli.environment.modulePath, Object.assign({}, cli.environment, options))).then(result => this.installDependencies(cli.environment.cwd, result.install)).then(async () => {
      if (cli.options.link) {
        _Logger.default.info('Linking atscm...');
      ...
      _Logger.default.info('Created new project at', _Logger.default.format.path(cli.environment.cwd));
    });
  }
  /**
