...
class AtSCMCli extends _liftoff2.default {
  ...
  _reportCliError(err) {
    ...
    if (err instanceof _UsageError2.default) {
      ...
      if (err instanceof SyntaxError && this._failedRequires.length) {
        _Logger2.default.info((0, _chalk.yellow)(`You may have to install the '${this._failedRequires[0]}' module.`));
        _Logger2.default.info(['Other failed requires:', ...this._failedRequires].join(`${_os.EOL} - `));
      }
    }
  ...
  constructor(argv = []) {
    ...
    this.on('requireFail', function (name) {
      ...
      _Logger2.default.debug(_Logger2.default.colors.red('Failed to load external module'), _Logger2.default.colors.magenta(name));
    });
     ...
     * @type {Object}
     */
    this.options = (0, _yargs2.default)(argv).version(false).help(false).env('ATSCM').option(_Options.GlobalOptions).fail((msg, e, y) => {
    ...
    }, () => this.command = command), (0, _yargs2.default)().env('ATSCM').usage('Usage: $0 [cmd=run]').version(false).options(_Options.GlobalOptions).global(globalOptionNames).strict().help('help', _Options2.default.help.desc).alias('help', 'h'));
  }
